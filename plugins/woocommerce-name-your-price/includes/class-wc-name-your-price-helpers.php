...
class WC_Name_Your_Price_Helpers {
	...
	public static function get_price_string( $product, $type = 'suggested' ) {
		...
		// minimum or suggested price/period
		if( 'minimum' == $type ){
		...
		if( 'woocommerce_get_price_html' != current_filter() && class_exists( 'WC_Subscriptions_Product' ) && WC_Subscriptions_Product::is_subscription( $product ) ) {

			// if a billing period was passed ( then it is a variable billing product and need to create our own string )
			if( $product->is_type( 'subscription' ) && $variable_billing ) { 

				$html = sprintf( _x( ' %s / %s', 'Variable subscription price, ex: $10 / day', 'wc_name_your_price' ), wc_price( $price ), WC_Subscriptions_Manager::get_subscription_period_strings( 1, $period ) );
	...
	public static function get_posted_period( $product_id, $prefix = false ) {
		...
		$posted_period = isset( $_REQUEST['nyp-period' . $prefix] ) ?  $_REQUEST['nyp-period' . $prefix] : '';
	...
	public static function get_data_attributes( $product_id, $prefix  = null ) {
		...
		$posted = self::get_posted_price( $product_id, $prefix );

		$initial = self::get_initial_price( $product_id );

		$minimum = self::get_minimum_price( $product_id ); 

		$price = $posted ? $posted : $initial;
