...
Description: Plugin for accepting recurring PayPal donations via shortcode
Author: wpecommerce
Version: 1.1
	...
	function dntplgn_settings_page() { 
		...
		<div class="wrap">
			...
			<div class="dntplgn_description_shortcode_block">
				<p>
                                    <?php _e( 'You can use the [dntplgn] shortcode in a wp post, page or sidebar text widget to show the recurring donation form.', 'donateplugin' ); ?>
                                </p>
                                <p>
                                    <?php _e( 'Alternatively, you can use the shortcode with custom parameters to customize the donation amount.', 'donateplugin' ); ?> Example shortcode: <br />
                                    <code>[dntplgn recurring_amt1="10.00" recurring_amt2="50.00" recurring_amt3="200.00" item_name="Donation for a cause" paypal_email="paypalemail@example.com"]</code>
                                </p>
				<p>
                                    <?php _e( 'Default parameters of the shortcode', 'donateplugin' ); ?>:</br >
		...
		<div id="tabs" class="dntplgn_form_wrapper">
			...
			<div id="tabs-1">
				...
				<form class="dntplgn_donate_monthly"  action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top" class="dntplgn_form">
					...
					<label for="first_button"> $<?php echo $dntplgn_atts['recurring_amt1']; ?> <span>(p/mês)</span></label>
					<input id="second_button" type="radio" name="a3" value="<?php echo $dntplgn_atts['recurring_amt2']; ?>" />
					<label for="second_button"> $<?php echo $dntplgn_atts['recurring_amt2']; ?> <span>(p/mês)</span></label>
					<input id="third_button" type="radio" name="a3" value="<?php echo $dntplgn_atts['recurring_amt3']; ?>" />
					<label for="third_button"> $<?php echo $dntplgn_atts['recurring_amt3']; ?> <span>(p/mês)</span></label>
					<input id="fourth_button" type="radio" name="a3" value="other" />
					<label for="fourth_button"> <?php _e( 'Outro', 'donateplugin' ); ?> <span>(p/mês)</span></label></br>
					...
					<input type="hidden" name="on0" value="Contribua" />
					<input type="hidden" name="os0" value="Mensal" />
					<input class="dntplgn_submit_button" type="submit" name="submit"  value="<?php _e( 'Contribua', 'donateplugin' ); ?>" alt="PayPal - The safer, easier way to pay online!" />
