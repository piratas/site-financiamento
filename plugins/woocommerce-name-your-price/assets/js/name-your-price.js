		...
		$( this ).on( 'woocommerce-nyp-update', function() {
			...
			if ( annual_minimum > 0 ){
				...
				if( form_annulualized_price < annual_minimum ){
					...
					error_price = woocommerce_nyp_format_price( error, woocommerce_nyp_params.currency_symbol ) + ' / ' + $nyp_period.find('option[value="' + form_period + '"]').text();
			...
			// compare price directly
				...
				error = min_price;
				error_price = woocommerce_nyp_format_price( error, woocommerce_nyp_params.currency_symbol );
		...
		if ( $( this ).hasClass( 'variations_form' ) ) {
			...
			.on( 'found_variation', function( event, variation ) {
				...
				if ( typeof variation.is_nyp != undefined && variation.is_nyp == true ) {
					...
					initial_price_num = woocommerce_nyp_unformat_price( variation.initial_price );

					// get the minimum price
					minimum_price = woocommerce_nyp_unformat_price( variation.minimum_price );

					// maybe auto-format the input
					if( $.trim( initial_price ) != '' ){
						$nyp_input.val( woocommerce_nyp_format_price( initial_price_num ) );
					}
